...
void RenderProgress::update(int processed) {
    ...
    std::visit(
        [&](auto &&state) {
            ...
            if constexpr (std::is_same_v<T, _PlainState>) {
                state.completedWork += processed;
                current = state.completedWork;
                last = state.lastPrinted;
            } else if constexpr (std::is_same_v<T, _AtomicState>) {
                state.completedWork.fetch_add(processed, std::memory_order_relaxed);
                current = state.completedWork.load(std::memory_order_relaxed);
