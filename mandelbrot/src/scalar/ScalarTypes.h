...
#define SCALAR_SYM_F(a) _EVAL(a)
#define SCALAR_SYMS_F(a) _CONCAT2(a, d)
#else
typedef float scalar_full_t;
#define SCALAR_SYM_F(a) _CONCAT2(a, f)
#define SCALAR_SYMS_F SCALAR_SYM_F
#endif

typedef float scalar_half_t;
#define SCALAR_SYM_H(a) _CONCAT2(a, f)
#define SCALAR_SYMS_H SCALAR_SYM_H
...
#define FUNC1_F(name, x) SCALAR_SYM_F(name)(CAST_F(x))
#define FUNC1_H(name, x) SCALAR_SYM_H(name)(CAST_H(x))

#define FUNCS_F(name, ...) SCALAR_SYMS_F(name)(__VA_ARGS__)
#define FUNCS_H(name, ...) SCALAR_SYMS_H(name)(__VA_ARGS__)

#define FUNC2_F(name, a, b) SCALAR_SYM_F(name)(CAST_F(a), CAST_F(b))
#define FUNC2_H(name, a, b) SCALAR_SYM_H(name)(CAST_H(a), CAST_H(b))
