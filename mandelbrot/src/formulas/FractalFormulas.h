...
static inline void formula(number_t cr, number_t ci,
    ...
    if constexpr (N <= 0) {
    ...
    } else if constexpr (static_cast<int>(N) == N) {
        number_t zrzi = NUM_MUL(zr, zi);
        ...
        number_t rp = NUM_POW(mag, NUM_DIV(N, NUM_CONST(2.0)));
...
static inline void derivative(number_t zr, number_t zi,
    ...
    if constexpr (N <= 0) {
        ...
        number_t pow = NUM_SUB(NUM_VAR(N), NUM_CONST(1.0));
        number_t rp = NUM_POW(mag, NUM_DIV(pow, NUM_CONST(2.0)));

        number_t theta = NUM_ATAN2(zi, zr);
        number_t angle = NUM_MUL(pow, theta);
