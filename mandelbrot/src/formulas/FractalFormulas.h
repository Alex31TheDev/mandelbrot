...
static FORCE_INLINE void formula(const mpfr_t &cr, const mpfr_t &ci,
    const mpfr_t &zr, const mpfr_t &zi,
    const mpfr_t &zr2, const mpfr_t &zi2, const mpfr_t &mag,
    mpfr_t &new_zr, mpfr_t &new_zi) {
    ...
    if (N <= 0) {
        ...
        mpfr_t *zrzi = mul(&zr, &zi);
        mpfr_set(new_zr, *add(sub(&zr2, &zi2), &cr), ROUNDING);
        mpfr_set(new_zi, *add(add(zrzi, zrzi), &ci), ROUNDING);
