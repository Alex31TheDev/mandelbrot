...
inline __m128 normCos_vec(__m128 x) {
    __m128 t = _mm_sub_ps(x, f_pi_2);
    __m128 c = _mm_cos_ps(t);
    return _mm_mul_ps(_mm_add_ps(c, f_one), f_half);
}

inline void getColorPixel_vec(__m128 val, __m128 &outR, __m128 &outG, __m128 &outB) {
...
inline __m128i pixelToInt_vec(__m128 val) {
...
inline void setPixelsMasked_vec(uint8_t *pixels, int &pos, int width, __m128 active, __m128 R, __m128 G, __m128 B) {
...
__m128 getSmoothIterVal_vec(__m128 iter, __m128 mag) {
...
__m128 getLightVal_vec(__m128 zr, __m128 zi, __m128 dr, __m128 di) {
